...
# Introduction
Usually academic teaching programme lags behind the real world requirements - we have learned that this time-gap could be as big as two years. In order to remedy this quite excessive time lag [prof. Przemysław Biecek](http://biecek.pl/) from MiNI (faculty of Mathematics and Informational Studies) at Warsaw University of Technology decided to assign us (interns at MI2 group) a task – extract and identify skills needed by data science industry in order to align teaching with the current needs.

# Masterplan

Although the idea may appear as a simple task - it wasn't. We would have never made it if not for the professor's general idea about how things should be done. The main concept was to download a lot of job offers, decide if offer “is/is not data scientist” and tag it, train machine learning model on downloaded data, predict if fresh/“unseen” by the model offer is from the DS industry or not and extract most popular/needed skills from all DS offers. The plan can be summarised as follows:
...
```{r}
######### nowy plot pod spodem
```


```{r}
skillsGroupsDS <- read_csv("data/prog_tag_skills_table_names_DS.csv")

for (i in seq_len(nrow(skillsGroupsDS))) {
  skillsGroupsDS$skill_multip[i] <- ifelse(
    skillsGroupsDS$skill_multip[i] == 1, skillsGroupsDS$df_full_n[i], NA
  )
  skillsGroupsDS$DS_skill[i] <- ifelse(
    skillsGroupsDS$DS_skill[i] == 1, skillsGroupsDS$df_full_n[i], NA
  )
  skillsGroupsDS$Prog_skill[i] <- ifelse(
    skillsGroupsDS$Prog_skill[i] == 1, skillsGroupsDS$df_full_n[i], NA
  )
  skillsGroupsDS$DB_skill[i] <- ifelse(
    skillsGroupsDS$DB_skill[i] == 1, skillsGroupsDS$df_full_n[i], NA
  )
  skillsGroupsDS$Universal_skill[i] <- ifelse(
    skillsGroupsDS$Universal_skill[i] == 1, skillsGroupsDS$df_full_n[i], NA
  )
}

skillsGroupsDS <- skillsGroupsDS %>%
  filter(df_full_n != "id") %>%
  as.list() %>%
  tail(4) %>%
  lapply(function(x) {
...
sumTabDS <- read_csv("data/bigKWCountTabDS_dates.csv") %>% 
          select(-id) 
...
w2c <- words2CloudDS %>%
  filter(words != "Analitics_analysis" & words != "Computer_science" & words != "DS_skill" & words != "Prog_skill" & words != "Universal_skill" &  words != "DB_skill") %>%
  mutate(words = words %>% tolower())
wordcloud(
  w2c$words,
  w2c$freq,
...
tabsetPanel(
  ...
  tabPanel("Importance of variables vs their popularity - learning dataset", img(src = "plots/cvRF/importanceVsAppereance.png")),
  tabPanel("Importance of variables vs their popularity - complete dataset", img(src = "plots/cvRF/importanceVsAppearanceBigDF.png"))
)
```
